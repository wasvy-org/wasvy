...
**Module Plan**:
The canonical, language-neutral description carried by exactly one Module Artifact, defining its Module identity, systems, scheduling, invocation shape, state contract, dependencies, capabilities, and resource requirements. A Module Instance's effective Plan is the Plan of its active Generation's Artifact.
_Avoid_: Runtime registration, handwritten manifest, module-wide manifest
